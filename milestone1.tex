...
\begin{enumerate}\scriptsize
    \item \textbf{280-cycle overhead floor} --- raw stencil compute is 28 cycles, but sync/async ops add $\geq$250 cycles per stage. Adding this floor reduced stencil MAPE from 146\% to 3.7\%.
    \item \textbf{Occupancy loss is sub-linear} --- 3$\to$1 blocks/SM costs only $\sim$16\% throughput, not 42\%.
    \item \textbf{L40S absorbs most workloads} --- 112\,MB effective L2 means $W_{\text{conc}} < C_{\text{L2}}$ for nearly all configs $\to$ pipeline barely helps.
\end{enumerate}

\vspace{0.3em}
\centering\small
\textbf{Bottom line:} $\sim$10\% MAPE across 4 architectures with zero fitted parameters.
\end{column}
\end{columns}
\end{frame}
